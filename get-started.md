...
{% hint style="success" %}
本指南将跳过一些概念性基础知识，以便让您尽快上手。请查看[概念](concepts.md)，以深入了解 Passwordless.dev 使用的理念。
{% endhint %}
...
选择**创建应用程序**按钮，并为新的应用程序提供**应用程序名称**和**描述**。将为每一个应用程序生成一组 [API 密钥](concepts.md#api-keys)。您将使用这些 API 密钥对 Passwordless.dev API 进行身份验证。把您的公钥和私钥保存在安全的地方，比如 [Bitwarden Secrets Manager](https://help.ppgg.in/secrets-manager/secrets-manager-overview)。
